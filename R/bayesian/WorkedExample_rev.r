...
mu_b <- 0.015 # background mortality rate hard-coded as 0.015
v_mu <- c(0,0,mean(post_sir[,"mu_e"]),mean(post_sir[,"mu_l"]),mean(post_sir[,"mu_t"]))+mu_b
surv = 1/(v_mu[3]+mean(post_sir[,"p"]))+ mean(post_sir[,"p"])/(v_mu[3]+mean(post_sir[,"p"]))*(1/v_mu[4])
# Treatment volume at 30 years
tx = Calib$trace0[30*12,5]
# Calibrated parameters compared to prior
mean(post_sir[,"b"])    #mean(samp_i[,"b"])
mean(post_sir[,"mu_e"]) #mean(samp_i[,"mu_e"])
mean(post_sir[,"mu_l"]) #mean(samp_i[,"mu_l"])
mean(post_sir[,"mu_t"]) #mean(samp_i[,"mu_t"])
mean(post_sir[,"rho"])  #mean(samp_i[,"rho"])
mean(post_sir[,"p"])    #mean(samp_i[,"p"])
mean(post_sir[,"r_l"])  #mean(samp_i[,"r_l"])
